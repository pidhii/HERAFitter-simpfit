...
public:
   ...
   enum EMuX {
	...
	const
#endif
	double TWOPI = 6.28318530717958647692528,
   		   TWOPISQR = 39.47841760435743447533796;
...
public:
   ...
   int GetIExpUnit() const {
      return Ipublunits;
   };                       // exponent of xs units (like -12 for pb)
   string GetScenarioName() const {
      return ScenName;
   };                  // Get Scenario/Table name
   vector < string > GetScenarioDescription() const {
      return ScDescript;
   ...
   vector < string > GetDimensionLabel() const {
      return DimLabel;
   };      // Get label for measured dimensions
   vector < int > GetIDiffBin() const {
      return IDiffBin;
   };               // Get number of differential bins
   vector < vector < double > > GetLowBinEdge() const {
      return LoBin;
   }; // Get Lower Bin edge [ObsBin][DiffBin]
   vector < vector < double > > GetUpBinEdge() const {
      return UpBin;
   };  // Get Upper Bin edge [ObsBin][DiffBin]
   ...
   string GetScaleDescription(int scalen=0) const {
      return BBlocksSMCalc[0][0]->ScaleDescript[0][scalen];
   };             // Description of renormalization and facorization scale choice
   ...
   bool GetIsFlexibleScaleTable() const {
...
protected:
   ...
   FastNLOBlockB* B_NLO() {
      return BBlocksSMCalc[0][1];
   };
   FastNLOBlockB* B_LO() {
      return BBlocksSMCalc[0][0];
   };
