...
C Store FCN flag in a common block:
      ...
      if(iflag .ne. 1 .and. CIdoSimpFit .and.
     $   TRIM(CIsimpFitStep) .eq. "SimpFit") then
            call simpfcn(g_dummy, chi2out, parminuit, iflag)
            return
      end if

      if (CIDoSimpFit.and.TRIM(CIsimpFitStep).eq."CalcDerivatives") then
            chi2out = 42 ! hack 
            return
      end if
...
*     ---------------------------------------------------------
      ...
      if(CIDoSimpFit .and. iflag .ne. 1 .and. 
     $   TRIM(CIsimpFitStep) .eq. "SimpFit") then
            goto 99
      end if
      ...
      do idataset=1,NDATASETS
         if(NDATAPOINTS(idataset).gt.0) then
            call GetTheoryForDataset(idataset)
             ...
             write (Msg,
     $  '(''W: Data set '',i2,'' contains no data points, will 
     $  be ignored'')') idataset
...
C LW 29.07.16 Check final chi2 calculation:
      if (iflag.eq.3) then
        print *,' LW chi2 test:'
        print *,'    fchi2 = ',fchi2
        print *,'    DeltaLength = ',DeltaLength
        print *,'    TempChi2 = ',TempChi2
...
C !> Store also type of systematic source info
          ...
          write(85,'(I5,''  '',A35,'' '',F9.6,''   +/-'',F9.6,A10,2A2)')
